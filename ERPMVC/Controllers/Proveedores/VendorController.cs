...
namespace ERPMVC.Controllers
{
    ...
    public class VendorController : Controller
    {
        ...
        public VendorController(ILogger<VendorController> logger, IOptions<MyConfig> config, IHttpContextAccessor httpContextAccessor)
        {
            this.config = config;
            this._logger = logger;
        ...
        {
            Vendor _Invoice = new Vendor();
            try
            {
                string baseadress = config.Value.urlbase;
                HttpClient _client = new HttpClient();
                _client.DefaultRequestHeaders.Add("Authorization", "Bearer " + HttpContext.Session.GetString("token"));
                var result = await _client.PostAsJsonAsync(baseadress + "api/Vendor/GetVendorByRTN", _Invoicep);
                string valorrespuesta = "";
                if (result.IsSuccessStatusCode)
                {
                    valorrespuesta = await (result.Content.ReadAsStringAsync());
                    _Invoice = JsonConvert.DeserializeObject<Vendor>(valorrespuesta);
                }
                if (_Invoice == null)
                {
                    _Invoice = new Vendor();
