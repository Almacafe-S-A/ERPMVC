...
namespace ERPMVC.Controllers
{
    ...
    public class JournalEntryConfigurationController : Controller
    {
        ...
        public async Task<ActionResult> pvwJournalEntryConfiguration(Int64 Id = 0)
        {
            JournalEntryConfiguration _JournalEntryConfiguration = new JournalEntryConfiguration();
            try
            {
                string baseadress = config.Value.urlbase;
                HttpClient _client = new HttpClient();
                _client.DefaultRequestHeaders.Add("Authorization", "Bearer " + HttpContext.Session.GetString("token"));
                var result = await _client.GetAsync(baseadress + "api/JournalEntryConfiguration/GetJournalEntryConfigurationById/" + Id);
                ...
                {
                    _JournalEntryConfiguration = new JournalEntryConfiguration();
                }
            }
            catch (Exception ex)
            {
