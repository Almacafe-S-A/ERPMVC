...
namespace ERPMVC.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        {
            ViewData["permisoAgregar"] = _principal.HasClaim("Catalogos.Servicios.Agregar", "true");
            ViewData["permisoEditar"] = _principal.HasClaim("Catalogos.Servicios.Editar", "true");
            ViewData["permisoEliminar"] = _principal.HasClaim("Catalogos.Servicios.Eliminar", "true");
            return View();
        }
        ...
        public async Task<DataSourceResult> GetActivos([DataSourceRequest] DataSourceRequest request)
        {
            ...
            try
            {
                ...
                {
                    valorrespuesta = await (result.Content.ReadAsStringAsync());
                    _Product = JsonConvert.DeserializeObject<List<Product>>(valorrespuesta).Where(w=> w.IdEstado==1).ToList();
                    _Product = _Product.OrderByDescending(q => q.ProductId).ToList();
                }
            }
            catch (Exception ex)
